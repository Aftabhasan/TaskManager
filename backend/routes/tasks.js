...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const member = await db.prepare("SELECT userId FROM project_members WHERE projectId = ? AND userId = ?").get(projectId, req.user.id);
      ...
      if (task.assignedTo) task.assignedTo = await db.prepare("SELECT id, name, email FROM users WHERE id = ?").get(task.assignedTo);
      task.createdBy = await db.prepare("SELECT id, name, email FROM users WHERE id = ?").get(task.createdBy);
...
router.get("/", async (req, res) => {
  try {
    ...
    if (req.query.project) {
      const member = await db.prepare("SELECT userId FROM project_members WHERE projectId = ? AND userId = ?").get(req.query.project, req.user.id);
      if (!member) return res.status(403).json({ error: "Not a member of this project" });
      tasks = await db.prepare("SELECT * FROM tasks WHERE projectId = ? ORDER BY createdAt DESC").all(req.query.project);
    } else {
      ...
      tasks = await db.prepare(`SELECT * FROM tasks WHERE projectId IN (${projectIds.map(() => "?").join(",")}) ORDER BY createdAt DESC`).all(...projectIds);
    }
...
router.get("/:id", [param("id").isInt()], async (req, res) => {
  try {
    ...
    const member = await db.prepare("SELECT userId FROM project_members WHERE projectId = ? AND userId = ?").get(task.projectId, req.user.id);
    if (!member) return res.status(403).json({ error: "Not a member of this project" });
    if (task.assignedTo) task.assignedTo = await db.prepare("SELECT id, name, email FROM users WHERE id = ?").get(task.assignedTo);
    task.createdBy = await db.prepare("SELECT id, name, email FROM users WHERE id = ?").get(task.createdBy);
    task.project = await db.prepare("SELECT id, name FROM projects WHERE id = ?").get(task.projectId);
...
router.put("/:id", [param("id").isInt()], async (req, res) => {
  try {
    ...
    if (updated.assignedTo) updated.assignedTo = await db.prepare("SELECT id, name, email FROM users WHERE id = ?").get(updated.assignedTo);
    updated.createdBy = await db.prepare("SELECT id, name, email FROM users WHERE id = ?").get(updated.createdBy);
    updated.project = await db.prepare("SELECT id, name FROM projects WHERE id = ?").get(updated.projectId);
